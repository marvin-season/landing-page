...
async function main() {
  ...
  if (command === 'export') {
    ...
    if (!options.output) {
      console.error('Usage: export --output <file.csv> [--provider <provider>] [--apiKey <key>] [--appId <id>] [--sourceLang <lang>] [--localesDir <dir>]');
      process.exit(1);
    }
    ...
    if (!options.csv || !options.po) {
      console.error('Usage: import --csv <file.csv> --po <file.po> [--localesDir <dir>]');
      process.exit(1);
    }
    ...
    console.error('Usage:');
    console.error('  export --output <file.csv> [--provider <provider>] [--apiKey <key>] [--appId <id>] [--sourceLang <lang>]');
