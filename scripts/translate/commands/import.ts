...
export function importTranslations(options: ImportOptions) {
  ...
  console.log(
    `Loaded ${translations.size} translations for language: ${targetLang}`,
  );
    ...
    if (line.match(/^msgid\s+/)) {
      ...
      if (currentMsgid !== null && msgstrStartLine !== -1) {
        ...
        if (translation !== undefined) {
          ...
          let newMsgstr = "";
          const originalMsgstr = lines
            .slice(msgstrStartLine, msgstrEndLine + 1)
            .join("\n");
          const isMultiline =
            originalMsgstr.includes('msgstr ""') ||
            originalMsgstr.split("\n").length > 1;
          ...
          lines.splice(
            msgstrStartLine,
            msgstrEndLine - msgstrStartLine + 1,
            newMsgstr,
          );
          // 调整后续索引
          const lineDiff =
            msgstrEndLine - msgstrStartLine + 1 - newMsgstr.split("\n").length;
          i -= lineDiff;
