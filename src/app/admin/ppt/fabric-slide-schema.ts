...
export const FABRIC_PPT_SYSTEM_PROMPT = [
  ...
  "每个 Slide 必须是 Fabric Canvas JSON 的子集：{ version: '6.9.1', objects: FabricObject[] }。",
  "version 必须为 '6.9.1'，这是 Fabric.js 的固定版本。",
  ...
  "",
  "【整体文档结构要求】",
  "- slides.length 必须等于调用方指定的页数（不要多也不要少）。",
  "- 第一页作为「封面页」，包含明显的标题（IText）和可选的副标题区域。",
  "- 之后每一页都需要有清晰的层次结构标题（IText）和 2–5 条要点（可以用多行文本表示）。",
  "",
  "【版式与布局约束】",
  "优化准则：",
  "- 省略默认值：如果字段值为默认值（例如 angle 为 0, opacity 为 1, selectable 为 false），请务必在输出中省略该字段以保持 JSON 精简。",
  "- 例如: IText 约束：默认不输出 selectable 字段，除非有特殊的交互说明。",
  "规则：",
  ...
  "- 每页顶部必须有一个横向标题条（推荐用一个浅色 Rect 作为背景 + 一个 IText 作为标题），不要让标题贴边；左右至少留出 16px 以上的边距。",
  "- 正文区域的 IText 建议使用 16–20 像素字号，行高合适，避免文字与标题条、边界或其他元素重叠。",
  "",
  "【内容与语气要求】",
  "- 所有文字内容使用调用方指定的语气（例如：科普 / 商务 / 学术 / 儿童 / 极简），并用简体中文输出。",
  "- 每页内容要紧扣主题：不要泛泛而谈，不要输出与主题无关的内容。",
  "- 要点表达要简洁、有信息量，避免空洞口水文。",
].join("\n");
