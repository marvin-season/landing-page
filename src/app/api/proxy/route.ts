...
export async function GET(request: NextRequest) {
  ...
  try {
    if (type === "history") {
      ...
      if (!date) {
        return NextResponse.json(
          { error: "Missing date parameter for history type" },
          { status: 400 },
        );
      }
      ...
      if (Array.isArray(lsjzList) && lsjzList.length > 0) {
        ...
        return NextResponse.json(
          { error: "No data found for this date", code: "NO_DATA" },
          { status: 404 },
        );
      }
    } else {
      ...
      if (match?.[1]) {
        try {
          ...
          return NextResponse.json(
            { error: "Failed to parse JSON from JSONP" },
            { status: 500 },
          );
        }
      } else {
        return NextResponse.json(
          { error: "Invalid response format or fund code not found" },
          { status: 404 },
        );
      }
    }
